...
print qq{
    <div class="main_inner_cntnr">
        ...
        <div class="main_inner_cntnr_middle">
            <div class="head">Easy Recipes</div>
...
if ($action) {
    ...
    if (!@recipe_id) {
        print qq{
            ...
            <a href="$GLOB{settings}{files_url}/admin/easy_recipes.cgi">Back</a>
        };
    }
    else {
        $dbh->do(qq{
    ...
    if ($result) {
        my %checked;
        $checked{easy_recipe}{1} = qq{ checked="checked"};
        ...
        while (my ($recipe_id, $name, $easy_recipe) = $sth->fetchrow_array) {
            print qq{
                    <div style="position:relative; float:left; width:90%; height:32px; text-align:left;">
                        <span style="width:25px;"><input type="checkbox" name="recipe_id" value="$recipe_id"$checked{easy_recipe}{$easy_recipe} /></span>
