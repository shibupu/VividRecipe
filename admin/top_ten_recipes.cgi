...
print qq{
    <div class="main_inner_cntnr">
        ...
        <div class="main_inner_cntnr_middle">
            <div class="head">Top 10 Recipes</div>
...
if ($action) {
    ...
    if (!@recipe_id) {
        print qq{
            Please select the recipes.
            <a href="$GLOB{settings}{files_url}/admin/top_ten_recipes.cgi">Back</a>
        };
    }
    elsif (@recipe_id > 10) {
        print qq{
            Please select maximum 10 recipes.
            <a href="$GLOB{settings}{files_url}/admin/top_ten_recipes.cgi">Back</a>
        };
    }
    else {
    ...
    if ($result) {
        my %checked;
        $checked{top_ten_recipe}{1} = qq{ checked="checked"};
        ...
        while (my ($recipe_id, $name, $top_ten_recipe) = $sth->fetchrow_array) {
            print qq{
                    <div style="position:relative; float:left; width:90%; height:32px; text-align:left;">
                        <span style="width:25px;"><input type="checkbox" name="recipe_id" value="$recipe_id"$checked{top_ten_recipe}{$top_ten_recipe} /></span>
        ...
        <script type="text/javascript">
            function validateForm() {
                ...
                if (checked_recipes == 0) {
                    alert('Please select the recipes');
                    return false;
                }

                if (checked_recipes > 10) {
                    alert('Please select maximum 10 recipes');
                    return false;
                }
